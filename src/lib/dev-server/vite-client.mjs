class HMRContext {
  ...
  invalidate(message) {
    ...
    this.hmrClient.logger.debug(
      `[vite nested] invalidate ${this.ownerPath}${message ? `: ${message}` : ""}`
    );
  }
  on(event, cb) {
    const addToMap = (map) => {
...
class HMRClient {
  ...
  async fetchUpdate(update) {
    ...
    return () => {
      ...
      this.logger.debug(`[vite nested] hot updated: ${loggedPath}`);
    };
  }
}
...
console.debug("[vite nested] connecting...");
...
try {
  ...
  if (!hmrPort) {
    fallback = () => {
      ...
      socket.addEventListener(
        ...
        () => {
          console.info(
            "[vite nested] Direct websocket connection fallback. Check out https://vite.dev/config/server-options.html#server-hmr to remove the previous connection error."
          );
        },
        { once: true }
      );
  ...
  console.error(`[vite nested] failed to connect to websocket (${error}). `);
}
function setupWebSocket(protocol, hostAndPath, onCloseWithoutOpen) {
  console.log('connect to', `${protocol}://${hostAndPath}`)
...
async function handleMessage(payload) {
  switch (payload.type) {
    case "connected":
      console.debug(`[vite nested] connected.`);
      ...
      await Promise.all(
        payload.updates.map(async (update) => {
          ...
          return new Promise((resolve) => {
            ...
            const removeOldEl = () => {
              el.remove();
              console.debug(`[vite nested] css hot updated: ${searchUrl}`);
              resolve();
            };
    ...
    case "error": {
      ...
      if (hasDocument) {
        ...
        if (enableOverlay) {
          ...
          console.error(
            `[vite nested] Internal Server Error
${err.message}
${err.stack}`
          );
        }
