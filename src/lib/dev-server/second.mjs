...
let{HTMLElement}=globalThis;if(!HTMLElement){HTMLElement=class{}}
...
const hmrClient = new HMRClient(console, {
    ...
    const importPromise = allbase_im(base + acceptedPathWithoutQuery.slice(1) + `?${ explicitImportRequired ? 'import&' : '' }t=${ timestamp }${ query ? `&${ query }` : '' }`);
...
async function waitForSuccessfulPing(socketProtocol2, hostAndPath, ms = 1000) {
